...
int main() {
    ...
    const std::string MC_FILE =
        "/data/whybee0a/user/gude_2/MC/20150212_MC_CTEQ6LL/MadGraph_hadded.root";
    ...
    const std::string MC_HISTO_ALL =
        "ZFinder/Combined Gen Cuts MC/0 All Events/z_mass_all";
    ...
    // The gen selection
    const std::string MC_HISTO_GEN_ACC =
        "ZFinder/Combined Gen Cuts MC/3 60 < GEN M_{ee} < 120/z_mass_all";
    ...
    //    return EXIT_FAILURE;
    //}
    TH1D* mc_histo_gen_acc;
    mc_tfile->GetObject(MC_HISTO_GEN_ACC.c_str(), mc_histo_gen_acc);
    if (!mc_histo_gen_acc) {
        std::cout << "Failed to load MC_HISTO_GEN_ACC" << std::endl;
        return EXIT_FAILURE;
    }
    TH1D* data_histo;
    ...
    const int MC_GEN_ACC_LOW = mc_histo_gen_acc->FindBin(60.);
    const int MC_GEN_ACC_HIGH = mc_histo_gen_acc->FindBin(120.);
    const double MC_COUNT_GEN_ACC = mc_histo_gen_acc->Integral(MC_GEN_ACC_LOW, MC_GEN_ACC_HIGH);

    // Calculate the final number
    //const double CROSS_SECTION = (MC_COUNT_ALL / MC_COUNT_ACC)
    //                             * (DATA_COUNT / LUMI);

    // Calculate the All to Gen correction
    const double ALL_TO_GEN = MC_COUNT_ALL / MC_COUNT_GEN_ACC;
    std::cout << "All / Gen Selection = " << ALL_TO_GEN << " = " << MC_COUNT_ALL << " / " << MC_COUNT_GEN_ACC << std::endl;
