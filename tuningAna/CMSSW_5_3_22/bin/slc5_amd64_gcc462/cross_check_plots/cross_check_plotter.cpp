...
void CrossCheckPlotter::plot(
    ...
    if (plot_config.binning.size() == 1) {
        ...
        //        data_histo
        //        );
        std::vector<double> new_binning = plot_config.binning;
    ...
    // If logx, then we need to make sure that x range doesn't include 0
    if (plot_config.logx && data_histo->GetXaxis()->GetXmin() <= 0) {
        const double X_MAX = data_histo->GetXaxis()->GetXmax();
        data_histo->GetXaxis()->SetRangeUser(0.002, X_MAX);
    }
    ...
    const double RATIO_OFFSET_FROM_1 = 0.04 + 0.015; // 0.02 is to avoid label clipping
    ...
    conf_map_.insert(
            config_pair(
                ...
                PlotConfig(
                    ...
                    //{}
                    {0.002, 0.004, 0.008, 0.012, 0.016, 0.020, 0.024, 0.029,
