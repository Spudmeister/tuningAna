...
def plot_same_normalized(histograms):
    ...
    chiSquared = 0.0
    #print sys.argv[1]+"phistar distribution histogram values:"
    for ibin in range(1, maxBin+1):
        chiSquared += ((data.GetBinContent(ibin) - histograms[0].GetBinContent(ibin))**2)/((data.GetBinError(ibin))**2)
        #print histograms[0].GetBinContent(ibin)
    print chiSquared
    p3 = c.cd(3)
    p3.SetPad(0,0,1,.15)
...
phi_1 = file1.Get("phistar/phistarBorn")
